...
function Job(job) {
  ...
  if(_.isArray(this.opts.origins))
    ...
    return this.cb("requires a 'origin'");

  for (var i = this.origins.length - 1; i >= 0; i--)
    if(!/^https?:\/\/[^\/]+$/.test(this.origins[i]))
      return this.cb("Invalid origin: '"+this.origins[i]+"', must be http[s]://host:port");
...
Job.prototype.ranSequence = function(err, results) {
  ...
  if((this.stopped || !this.timesup || this.results.length < this.running) &&
...
Job.prototype.runRequest = function(jar, reqObj, done) {
  ...
  var i = this.opts.sequence.indexOf(reqObj);
  ...
  var url = this.next(this.origins) + path;
